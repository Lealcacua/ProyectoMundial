...
public class ResultadosDAO {
    ...
    public boolean registrarResultados(Resultados resultado) {
        ...
        String sql = "INSERT INTO j_leal5.partidos (grupo, local, visitante, continente_local,continente_visitante,goles_local,goles_visitante) values("
                + "'" + resultado.getGrupo() + "', "
                + "'" + resultado.getLocal() + "', "
    ...
    public List<Resultados> getSelecciones() {
        ...
        String sql = "SELECT grupo,local, visitante, continente_local,continente_visitante,goles_Local,goles_visitante FROM j_leal5.partidos";
    ...
    public int totalPartidos() {
        int totalPartidos = 0;
        try {
            ResultSet result = BasedeDatos.ejecutarSQL("SELECT COUNT(*) AS total_partidos FROM j_leal5.partidos");
            if (result.next()) {
    ...
    public double promedioGolePorPatidos() {
        ...
        try {
            ResultSet result = BasedeDatos.ejecutarSQL("SELECT AVG(goles_local + goles_visitante) AS promedio_goles_por_partido FROM j_leal5.partidos");
            if (result.next()) {
    ...
    public DefaultTableModel partidosMasGoles() {
        String[] columnas = {"Equipo1", "Equipo2", "Goles-Equipo1", "Goles-Equipo2"};
        Object[][] matrizResultados = new Object[0][columnas.length];
        try {
            ResultSet result = BasedeDatos.ejecutarSQL("SELECT local as EQUIPO1, visitante as EQUIPO2, goles_local as GOLESEQUIPO1, goles_visitante as GOLESEQUIPO2 FROM j_leal5.partidos WHERE goles_local + goles_visitante = (SELECT MAX(goles_local + goles_visitante) FROM j_leal5.partidos)");
            while (result.next()) {
    ...
    public DefaultTableModel partidosMenosGoles() {
        ...
        Object[][] matrizResultados = new Object[0][columnas.length];
        try {
            ResultSet result = BasedeDatos.ejecutarSQL("SELECT local as EQUIPO1, visitante as EQUIPO2, goles_local as GOLESEQUIPO1, goles_visitante as GOLESEQUIPO2 FROM j_leal5.partidos WHERE goles_local + goles_visitante = (SELECT MIN(goles_local + goles_visitante) FROM j_leal5.partidos)");
            while (result.next()) {
                String equipo1 = result.getString("EQUIPO1");
                String equipo2 = result.getString("EQUIPO2");
                int golesEquipo1 = result.getInt("GOLESEQUIPO1");
    ...
    public int[] partidoGanadosEmpatados() {
        int partidosEmpatados = 0;
        int partidosGanados = 0;
        try {
            ResultSet result = BasedeDatos.ejecutarSQL("SELECT SUM(CASE WHEN goles_local > goles_visitante THEN 1 ELSE 0 END) + SUM(CASE WHEN goles_local < goles_visitante THEN 1 ELSE 0 END) AS partidos_con_ganador, SUM(CASE WHEN goles_local = goles_visitante THEN 1 ELSE 0 END) AS partidos_con_empate FROM j_leal5.partidos");
            if (result.next()) {
    ...
    public String[] seleccionesMasGoles() {
        ...
        try {
            ResultSet result = BasedeDatos.ejecutarSQL("SELECT seleccion FROM (SELECT local AS seleccion, SUM(goles_local) AS goles FROM j_leal5.partidos GROUP BY local union SELECT visitante AS seleccion, SUM(goles_visitante) AS goles FROM j_leal5.partidos GROUP BY visitante) t WHERE goles = (SELECT MAX(goles) FROM ( SELECT SUM(goles_local) AS goles FROM j_leal5.partidos GROUP BY local union SELECT SUM(goles_visitante) AS goles FROM j_leal5.partidos GROUP BY visitante) s)");
            while (result.next()) {
    ...
    public String[] seleccionesMenosGoles() {
        ArrayList<String> selecciones = new ArrayList<>();
        try {
            ResultSet result = BasedeDatos.ejecutarSQL("SELECT seleccion, goles FROM (SELECT local AS seleccion, SUM(goles_local) AS goles FROM j_leal5.partidos GROUP BY local union SELECT visitante AS seleccion, SUM(goles_visitante) AS goles FROM j_leal5.partidos GROUP BY visitante) t WHERE goles = (SELECT MIN(goles) FROM ( SELECT SUM(goles_local) AS goles FROM j_leal5.partidos GROUP BY local union SELECT SUM(goles_visitante) AS goles FROM j_leal5.partidos GROUP BY visitante) s)");
            while (result.next()) {
                selecciones.add(result.getString("seleccion"));
            }
        } catch (Exception e) {
    ...
    public String[] continentesMayorPuntuacion() {
        ArrayList<String> selecciones = new ArrayList<>();
        try {
    ...
    public String[] continentesMenorPuntuacion() {
        ArrayList<String> selecciones = new ArrayList<>();
        try {
    ...
    public List<Resultados> getResultadosBusqueda(String nombreSeleccion) {
        String sql = "SELECT grupo, local, visitante, continente_local, continente_visitante, goles_local, goles_visitante FROM j_leal5.partidos WHERE local LIKE ? OR grupo LIKE ? OR visitante LIKE ? OR continente_local LIKE ? OR continente_visitante LIKE ?";
        List<Resultados> resultados = new ArrayList<Resultados>();
        try {
